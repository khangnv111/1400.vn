    ...
    public class UserRoleDAOImpl : IUserRoleDAO
    {
        ...
        public int UserFunctionInsertList(int UserID, string ListRole, int CreateUserID, bool IsAdmin)
        {
            try
            {
                ...
                new DBHelper(Config.Site1400ConnectionString).ExecuteNonQuerySP("SP_UserFunction_Insert", pars);
        ...
        public int UserFunctionDelete(int UserID, int FunctionID)
        {
            try
            {
                ...
                new DBHelper(Config.Site1400ConnectionString).ExecuteNonQuerySP("SP_UserFunction_Delete", pars);
        ...
        public int UserFunctionDeleteList(int UserID, string ListFunction)
        {
            try
            {
                ...
                pars[2] = new SqlParameter("@_ResponseCode", SqlDbType.Int) { Direction = ParameterDirection.Output };
                new DBHelper(Config.Site1400ConnectionString).ExecuteNonQuerySP("SP_UserFunction_DeleteList", pars);
                return Convert.ToInt32(pars[2].Value);
            }
            catch (Exception ex)
            {
        ...
        public int UserFunctionDeleteAll(int UserID)
        {
            try
            {
                ...
                new DBHelper(Config.Site1400ConnectionString).ExecuteNonQuerySP("SP_UserFunction_DeleteAllByUserID", pars);
        ...
        public List<UserFunction> UserFunction_GetByUserID(int UserID)
        {
            try
            {
                ...
                pars[0] = new SqlParameter("@_UserID", UserID);
                var list = new DBHelper(Config.Site1400ConnectionString).GetListSP<UserFunction>("SP_UserFunction_GetByUserID", pars);
        ...
        public List<UserFunction> GetListUser_ByRoleFunctionID(int FunctionID, int IsGrant)
        {
            try
            {
                ...
                var list = new DBHelper(Config.Site1400ConnectionString).GetListSP<UserFunction>("SP_User_GetList_ByRoleFunctionID", pars);
                if (list == null || list.Count <= 0)
                    return new List<UserFunction>();
                return list;
            }
